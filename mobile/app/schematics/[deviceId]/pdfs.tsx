...
import { getSchematicsByDevice, generateTempPdfUrl, Schematic } from '@/api/schematic';
...
export default function SchematicsPdfListScreen() {
  const { deviceId, modelName } = useLocalSearchParams();
  ...
  useEffect(() => {
    if (deviceId) {
      getSchematicsByDevice(parseInt(deviceId as string))
        .then(setPdfs)
