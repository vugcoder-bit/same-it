...
export default function DevicesManagementScreen() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [devRes, typeRes] = await Promise.all([
            ...
            Toast.error('Failed to load devices and brands');
        } finally {
            setLoading(false);
        }
    };
            ...
            <View style={styles.actionRow}>
                ...
                <View style={styles.searchContainer}>
                    ...
                    <TextInput
                        ...
                        placeholder={`Search ${activeTab === 'types' ? 'brands' : 'devices'}...`}
                ...
                >
                    ...
                    <Text style={styles.addButtonText}>Add {activeTab === 'types' ? 'Brand' : 'Device'}</Text>
                </Pressable>
            </View>
                        ...
                        <View style={styles.imageUploadRow}>
                            <Pressable onPress={pickImage} style={styles.imagePlaceholder}>
                                {typeIcon || (editingId && deviceTypes.find(t => t.id === editingId)?.imageUrl) ? (
                                    <Image
                                        source={{ uri: typeIcon ? typeIcon.uri : `${apiClient.defaults.baseURL?.replace('/api', '')}/uploads/${deviceTypes.find(t => t.id === editingId)?.imageUrl}` }}
            ...
            <Modal transparent visible={deviceModalVisible} animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, !isDesktop && { width: '95%', padding: 20 }]}>
                        ...
                        <TextInput
                            style={styles.input}
                            placeholder="Device Name (e.g. Samsung A16e)"
                            value={deviceName}
